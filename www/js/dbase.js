...
function populateDB(tx) {
  ...
  tx.executeSql(
    "CREATE TABLE IF NOT EXISTS event (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, eventname TEXT NOT NULL, eventlocation TEXT NOT NULL, expectedguest INTEGER , tableno INTEGER)"
  );
  tx.executeSql(
    "INSERT INTO event(eventname, eventlocation, expectedguest, tableno) VALUES('', '', '', '')"
  );

  //create table guest
  tx.executeSql("DROP TABLE IF EXISTS guest");
  tx.executeSql(
    "CREATE TABLE IF NOT EXISTS guest (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, firstname TEXT NOT NULL, lastname TEXT NOT NULL, tableno INTEGER)"
  );
  tx.executeSql(
    "INSERT INTO guest(title, firstname, lastname, tableno) VALUES('', '', '', '')"
  );
  alert("guest created");
  ...
  tx.executeSql(
    "CREATE TABLE IF NOT EXISTS password (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, firstname TEXT NOT NULL, lastname TEXT NOT NULL, email TEXT NOT NULL, password TEXT NOT NULL)"
  );
  tx.executeSql(
    "INSERT INTO password(firstname, lastname, email, password) VALUES('', '', '', '')"
  );
...
function queryDB(tx) {
  tx.executeSql(
    "SELECT email, password FROM password  WHERE password=?",
    [signinPassword],
    querySuccess,
    errCB
  );
...
function querySuccess(tx, results) {
  ...
  if (len > 0) {
    ...
    if (
      results.rows.item(0)["password"] === signinPassword &&
      results.rows.item(0)["email"] === signinEmail
    ) {
